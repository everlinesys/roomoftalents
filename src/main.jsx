...
function App() {
    ...
    useEffect(() => {
      const onPop = () => {
        ...
        if (id) api(`/talents/${id}/public`).then(setSharedProfile).catch(() => setSharedProfile(null));
...
function Form({ mode, user, onAuth, onLogout, setToast, setModal }) {
    ...
    const [f, setF] = useState({ name: '', email: '', password: '', displayName: '', categories: [], location: '', contactVisible: true, age: '', sex: '', heightCm: '', bio: '', phone: '', photos: [], performanceLinks: [], skills: '' }); const set = (k, v) => setF(x => ({ ...x, [k]: v }));
    useEffect(() => { if (mode === 'profile' && user) api('/talents/me').then(d => { if (d.talent) setF(x => ({ ...x, ...d.talent, photos: d.talent.photos || [], performanceLinks: d.talent.performanceLinks || [], skills: (d.talent.skills || []).join(',') })); setStatus(d) }).catch(e => setError(e.message)); }, [mode, user]); useEffect(() => { if ((mode === 'subscribe' || mode === 'dashboard') && user) api('/subscriptions/mine').then(d => setSubscriptions(d.subscriptions || [])).catch(e => setError(e.message)); }, [mode, user]);
    async function submit(e) { e.preventDefault(); setError(''); setLoading(true); try { let d; if (mode === 'login') d = await api('/auth/login', { method: 'POST', body: JSON.stringify({ email: f.email, password: f.password }) }); else if (mode === 'register') d = await api('/auth/register', { method: 'POST', body: JSON.stringify({ name: f.name, email: f.email, password: f.password, role: kind }) }); else if (mode === 'verify') d = await api('/auth/verify-email', { method: 'POST', body: JSON.stringify({ email: f.email, code: f.code }) }); else if (mode === 'forgot') d = await api('/auth/forgot-password', { method: 'POST', body: JSON.stringify({ email: f.email }) }); else if (mode === 'reset') d = await api('/auth/reset-password', { method: 'POST', body: JSON.stringify({ email: f.email, code: f.code, password: f.password }) }); else if (mode === 'profile') d = await api('/talents', { method: 'POST', body: JSON.stringify({ ...f, contactVisible: f.contactVisible !== false, age: f.age || null, heightCm: f.heightCm || null, skills: f.skills.split(',').map(x => x.trim()).filter(Boolean), categories: f.categories, photos: f.photos || [], performanceLinks: (f.performanceLinks || []).filter(Boolean) }) }); if (d?.verificationRequired) { setModal('verify'); setToast(d.message); } else if (d?.user) onAuth(d); else if (mode === 'forgot') { setToast(d.message); setModal('reset'); } else if (mode === 'reset') { setToast(d.message); setModal('login'); } else { setStatus(d); setToast(d.message || 'Saved successfully.'); if (mode === 'profile' && !d.hasActivePro) setModal('subscribe'); } } catch (e) { if (e.data?.verificationRequired) { setModal('verify'); } setError(e.message) } finally { setLoading(false) } }
